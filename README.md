...
### Premium šabloni: izbor je slobodan, objavljivanje nije

`templates.required_plan_code` određuje koji paket šablon traži. Pravilo je
vezano za **stanje** pozivnice, a ne za samu radnju: dok je pozivnica nacrt, sme
da se primeni bilo koji šablon — korisnik treba da vidi šta bira i da ima razlog
da plati. Granica je objavljivanje, i brani se na tri mesta:

| Mesto | Šta sprečava |
|-------|--------------|
| `publishInvitation` | Objavu pozivnice čiji šablon paket ne pokriva |
| `switchInvitationTemplate` | Prelazak na nepokriven šablon kada je pozivnica **već objavljena** — inače bi se premium izgled dobio tako što se prvo objavi jeftinije, pa promeni šablon |
| `startCheckout` | Kupovinu paketa koji ne pokriva izabrani šablon, da novac ne bude uzet za objavu koja bi zatim bila odbijena |

Poređenje ide preko `sortOrder`, pa hijerarhiju menja administrator bez deploya;
paket sa `allTemplates` pokriva sve. Nepoznat `required_plan_code` namerno **ne**
prolazi (fail closed) — to je greška u podacima koja treba glasno da se vidi, a
tiho propuštanje bi značilo da premium šablon postane besplatan.

`getPublicationState` vraća `templateRequiresPlan`, pa interfejs može da objasni
**zašto** objavljivanje ne ide, umesto da samo onemogući dugme.
